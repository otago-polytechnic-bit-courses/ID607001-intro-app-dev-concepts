...
\lfoot{Practical\\Version 2, Semester Two, 2021}
\rfoot{\thepage}
...
\begin{itemize}
	\item \textbf{API} project can run locally without modification.
	\item Four \textbf{Models} containing at least four columns of data which you can interact with.
	\item \textbf{Controller} for each \textbf{Model} which have at least CRUD (create, read, write \& delete) functionality. For updating \& deleting, if the child table has a foreign key, you must use an on cascade update \& an on cascade delete. For example, if data in the parent table is updated, then data in the child/children table is updated as well.
	\item Apply custom validation when creating \& updating a record. 
	\item Seed each table using a \textbf{Seeder} \& \textbf{JSON} file.
	\item \textbf{Research:} Return an appropriate status code \& message when performing CRUD actions. For example, when a record is created, return 200 \& record successfully created. 
	\item Return data except for \textbf{id}, \textbf{created\_at} \& \textbf{updated\_at} using \textbf{API Resources}.
	\item \textbf{Research:} Return filtered, sorted \& paged \textbf{API} data from at least two \textbf{Models} using query parameters.
	...
	\item \textbf{API} project deployed to \textbf{Heroku}. The project must be usable i.e., a user should be able to perform requests to your \textbf{API}.
