...
Tasks are grouped into three tiers. **Core** tasks build the foundation. **Practice** tasks deepen your understanding. **Stretch** tasks are optional and open-ended. The **Project** task directly advances your assessment.
...
Module 04 replaces the hard-coded data in your backend with a real PostgreSQL database and Prisma ORM. The endpoints stay at the same URLs, so this frontend will continue to work - the only change will be that the data is now real and persistent.

Module 05 adds forms to the frontend so users can create, update, and delete records.
